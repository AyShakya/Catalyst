...
const SmoothScrollProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (isMobile) {
      // Skip Lenis for mobile, rely on native scroll
      return;
    }
    const lenis = new Lenis({
      ...
      smoothWheel: true,
      syncTouch: true,
      // automatically recalculate on resize
      autoResize: true,
    });
  ...
  // Reset scroll to top on route change (desktop only)
  useEffect(() => {
    if (lenisRef.current) {
  ...
  return (
    <div ref={scrollRef} className="smooth-scroll-wrapper min-h-screen overflow-hidden">
